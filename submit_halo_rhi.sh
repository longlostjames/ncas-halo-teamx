...
# Submit script for HALO RHI quicklooks
# Usage: ./submit_halo_rhi.sh START_DATE [END_DATE]
# Example: ./submit_halo_rhi.sh 20250626 20250630
...
echo "Submitting HALO RHI quicklook jobs"
echo "Year: $YEAR"
...
job_id=$(sbatch --array=${START_DOY}-${END_DOY} \
    --export=YEAR=${YEAR} \
    quicklook_halo_rhi_slurm.sh | awk '{print $4}')

if [ $? -eq 0 ]; then
    echo "Job submitted successfully with ID: $job_id"
    echo "Processing $num_days day(s)"
    echo "Monitor progress with: squeue -u $USER"
    echo "Check logs in: slurm_logs/halo_rhi_${job_id}_*.out"
    echo ""
